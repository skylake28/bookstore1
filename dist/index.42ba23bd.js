var shoppingCart = function() {
    ...
    obj.addItemToCart = function(name, price, count) {
        for(var i in cart)if (cart[i].name === name) {
            cart[i].count += count;
            saveCart();
            return;
        }
    ...
    obj.countCart = function() {
        // return total count
        var totalCount = 0;
        for(var i in cart)totalCount += cart[i].count;
        return totalCount;
    };
    obj.totalCart = function() {
        //return total cost
        var totalCost = 0;
        for(var i in cart)totalCost += cart[i].price * cart[i].count;
    ...
    obj.listCart = function() {
        ...
        for(var i in cart){
            ...
            var item = cart[i];
            var itemCopy = {};
            for(var p in item)itemCopy[p] = item[p];
...
//# sourceMappingURL=index.42ba23bd.js.map
